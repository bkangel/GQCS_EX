...
void IOCompletionPort::StartServer()
{
	...
	{		
		...
		m_hIOCP = CreateIoCompletionPort(
			(HANDLE)clientSocket, m_hIOCP, reinterpret_cast<ULONG_PTR>(m_pSocketInfo), 0
		);
...
volatile ULONGLONG count = 0;
volatile ULONGLONG doit = 0;
#define EX
#define COUNT 30000
#if defined(EX)
void IOCompletionPort::WorkerThread()
{
	...
	{
		BOOL ret = GetQueuedCompletionStatusEx(m_hIOCP, entries, ulCount, &ulNR, INFINITE, FALSE);
		...
		for (i = 0; i < ulNR; ++i)
		{
			pSocketInfo = (stSOCKETINFO*)entries[i].lpOverlapped;
			DWORD recvBytes = entries[i].dwNumberOfBytesTransferred;
			DWORD sendBytes = 0;
			DWORD dwFlags = 0;
			...
			if (pSocketInfo)
			{
				if (recvBytes == 0)
				{
					printf_s("[INFO] socket(%llx) 접속 끊김\n", pSocketInfo->socket);
			...
			if (::InterlockedIncrement(&count) > COUNT)
			{
				if (0 == ::InterlockedExchange(&doit, 1))
				{
					wchar_t msg[32];
					std::chrono::duration<double, std::deca> elapsed = std::chrono::high_resolution_clock::now() - begin;
					std::chrono::duration<double, std::milli> elapsed2 = elapsed;
					swprintf_s(msg, L"ex - %f, %f", elapsed.count(), elapsed2.count());
	...
	while (m_bWorkerThread)
	{
		...
		if (::InterlockedIncrement(&count) > COUNT)
		{
			if (0 == ::InterlockedExchange(&doit, 1))
			{
				...
				std::chrono::duration<double, std::deca> elapsed = std::chrono::high_resolution_clock::now() - begin;
				std::chrono::duration<double, std::milli> elapsed2 = elapsed;
				swprintf_s(msg, L"normal - %f, %f", elapsed.count(), elapsed2.count());
				::MessageBoxW(NULL, msg, L"", MB_OK);
			}
			return;
		}
